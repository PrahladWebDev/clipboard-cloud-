...
export class PairingService {
  ...
  async createSession(opts: { encrypted?: boolean; origin: string }) {
    ...
    await this.redis.set(SESSION_PREFIX + sessionId, JSON.stringify(record));
    await this.redis.set(CODE_PREFIX + code, sessionId);
    ...
    return { sessionId, code, qrDataUrl };
  }
  ...
  async joinByCode(code: string) {
    ...
    if (!sessionId) {
      throw new NotFoundException(
        'Invalid pairing code. Ask the other device to generate a new one.',
      );
    }
